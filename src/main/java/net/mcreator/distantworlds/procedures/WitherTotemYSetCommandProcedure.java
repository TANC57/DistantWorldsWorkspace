...
public class WitherTotemYSetCommandProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		...
		if (entity.hasPermissions(2)) {
			if ((new Object() {
				...
				{
					double _setval = Math.floor(DoubleArgumentType.getDouble(arguments, "position"));
					(new Object() {
						public Entity getEntity() {
							try {
								return EntityArgument.getEntity(arguments, "target");
				...
				if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_COMMANDBLOCKOUTPUT)) {
					if (entity instanceof Player _player && !_player.level().isClientSide())
						_player.displayClientMessage(
								Component.literal((Component.translatable("event.distant_worlds.command.data_changed").getString() + "" + new java.text.DecimalFormat("##").format(Math.floor(DoubleArgumentType.getDouble(arguments, "position"))))),
								false);
				}
			} else {
				if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_COMMANDBLOCKOUTPUT)) {
