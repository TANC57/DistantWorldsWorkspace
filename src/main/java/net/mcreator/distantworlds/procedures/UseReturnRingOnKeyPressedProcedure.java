...
public class UseReturnRingOnKeyPressedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double zTarget = 0;
		double xTarget = 0;
		if (entity instanceof LivingEntity lv ? CuriosApi.getCuriosHelper().findEquippedCurio(DistantWorldsModItems.RETURN_RING.get(), lv).isPresent() : false) {
			if (!(entity instanceof Player _plrCldCheck1 && _plrCldCheck1.getCooldowns().isOnCooldown(DistantWorldsModItems.RETURN_RING.get()))) {
				if (entity.isShiftKeyDown()) {
					if ((entity.level().dimension()) == Level.OVERWORLD && y > -64 && y <= 320 && !world.getBlockState(BlockPos.containing(x, y, z)).canOcclude() && !world.getBlockState(BlockPos.containing(x, y + 1, z)).canOcclude()) {
						...
						if (entity instanceof LivingEntity lv) {
							CuriosApi.getCuriosHelper().findCurios(lv, DistantWorldsModItems.RETURN_RING.get()).forEach(item -> {
								ItemStack itemstackiterator = item.stack();
								if (!(entity instanceof Player _plrCldCheck10 && _plrCldCheck10.getCooldowns().isOnCooldown(DistantWorldsModItems.RETURN_RING.get()))) {
						...
						if (!(entity instanceof ServerPlayer _plr15 && _plr15.level() instanceof ServerLevel
								&& _plr15.getAdvancements().getOrStartProgress(_plr15.server.getAdvancements().getAdvancement(new ResourceLocation("distant_worlds:return_point"))).isDone())) {
						...
						if (world != null) {
							...
							yTarget = (entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).ReturnRingY <= -64
									|| world.getBlockState(BlockPos.containing(xTarget, (entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).ReturnRingY, zTarget))
											.canOcclude()
									|| world.getBlockState(
											BlockPos.containing(xTarget, (entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).ReturnRingY + 1, zTarget))
											.canOcclude()
					...
					if (entity instanceof LivingEntity lv) {
						CuriosApi.getCuriosHelper().findCurios(lv, DistantWorldsModItems.RETURN_RING.get()).forEach(item -> {
							ItemStack itemstackiterator = item.stack();
							if (!(entity instanceof Player _plrCldCheck25 && _plrCldCheck25.getCooldowns().isOnCooldown(DistantWorldsModItems.RETURN_RING.get()))) {
								if (!(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
									{
					...
					if (!(entity instanceof Player _plrCldCheck32 && _plrCldCheck32.getCooldowns().isOnCooldown(DistantWorldsModItems.WITHER_TOTEM.get()))
							&& !(entity instanceof Player _plrCldCheck33 && _plrCldCheck33.getCooldowns().isOnCooldown(DistantWorldsModItems.CHARGED_WITHER_TOTEM.get()))) {
						if (entity instanceof Player _player)
