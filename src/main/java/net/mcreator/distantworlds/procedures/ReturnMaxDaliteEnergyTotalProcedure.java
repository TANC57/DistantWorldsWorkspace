...
public class ReturnMaxDaliteEnergyTotalProcedure {
	public static double execute(LevelAccessor world, Entity entity) {
			...
			return 0;
		{
			...
			entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
				capability.MaxDaliteEnergyTotal = _setval;
		...
		if (entity instanceof LivingEntity lv ? CuriosApi.getCuriosHelper().findEquippedCurio(DistantWorldsModItems.DALITE_STORAGE_INITIAL.get(), lv).isPresent() : false) {
			if (entity instanceof LivingEntity lv) {
				CuriosApi.getCuriosHelper().findCurios(lv, DistantWorldsModItems.DALITE_STORAGE_INITIAL.get()).forEach(item -> {
					...
					if (itemstackiterator.getOrCreateTag().getDouble("MaxEnergy") > 0) {
						{
							double _setval = (entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).MaxDaliteEnergyTotal
									+ itemstackiterator.getOrCreateTag().getDouble("MaxEnergy");
							entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
								capability.MaxDaliteEnergyTotal = _setval;
								capability.syncPlayerVariables(entity);
							});
		...
		return (entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).MaxDaliteEnergyTotal;
	}
}
