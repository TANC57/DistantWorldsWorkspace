...
public class VairisArmorPreventsFallDamageProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, DamageSource damagesource, Entity entity, double amount) {
		...
		if ((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.HEAD) : ItemStack.EMPTY).getItem() == DistantWorldsModItems.VAIRIS_HELMET.get()
				...
				&& (entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.FEET) : ItemStack.EMPTY).getItem() == DistantWorldsModItems.VAIRIS_BOOTS.get() && (damagesource.getEntity()) == null
				&& damagesource.is(DamageTypes.FALL)) {
			float BounceModifier = 0.5F;
			if ((world.getBlockState(BlockPos.containing(x, y - 0.2, z))).is(BlockTags.create(new ResourceLocation("distant_worlds:bouncy_blocks")))) {
				BounceModifier = 0.15F;
			}
			Amount = amount;
			if (event != null && event.isCancelable()) {
				event.setCanceled(true);
			}
			entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.FALL), entity), (float) (Amount / 2));
			entity.setDeltaMovement(entity.getDeltaMovement().add(0, BounceModifier, 0));
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.slime_block.fall")), SoundSource.PLAYERS, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.slime_block.fall")), SoundSource.PLAYERS, 1, 1, false);
				}
			}
		}
	}
