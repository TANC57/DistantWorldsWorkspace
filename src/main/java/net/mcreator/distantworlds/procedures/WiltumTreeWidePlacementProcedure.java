...
public class WiltumTreeWidePlacementProcedure {
			...
			if (AllowedHigh == 8) {
				if (Mth.nextInt(RandomSource.create(), 1, 4) == 1) {
					if (world instanceof ServerLevel _serverworld) {
						StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("distant_worlds", "wiltum_wide"));
						if (template != null) {
				...
				if (Math.random() < 0.7) {
					...
					world.setBlock(BlockPos.containing(x + 1, y, z), DistantWorldsModBlocks.WILTUM_LOG.get().defaultBlockState(), 3);
					world.setBlock(BlockPos.containing(x, y, z + 1), DistantWorldsModBlocks.WILTUM_LOG.get().defaultBlockState(), 3);
					world.setBlock(BlockPos.containing(x + 1, y, z + 1), DistantWorldsModBlocks.WILTUM_LOG.get().defaultBlockState(), 3);
					if (Mth.nextInt(RandomSource.create(), 1, 4) == 1) {
						if (world instanceof ServerLevel _serverworld) {
							StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("distant_worlds", "wiltum_wide"));
							if (template != null) {
