...
public class ReturnRingYGetCommandProcedure {
		...
		if (entity.hasPermissions(2)) {
			if ((new Object() {
				...
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal(("Y = " + new java.text.DecimalFormat("##").format(((new Object() {
						public Entity getEntity() {
							try {
								return EntityArgument.getEntity(arguments, "target");
							} catch (CommandSyntaxException e) {
					...
					}.getEntity()).getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).ReturnRingY))),
