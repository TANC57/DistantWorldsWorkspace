...
public class GarsaleCouchUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate) {
		if (!(!world.getEntitiesOfClass(GarsaleCouchRiddenEntity.class, AABB.ofSize(new Vec3((x + 0.5), y, (z + 0.5)), 0.5, 0.5, 0.5), e -> true).isEmpty())
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = DistantWorldsModEntities.GARSALE_COUCH_RIDDEN.get().spawn(_level, BlockPos.containing(x + 0.5, y, z + 0.5), MobSpawnType.MOB_SUMMONED);
