...
public class AgeEntitySetCommandProcedure {
			...
			if ((new Object() {
				...
				if ((new Object() {
					...
					if ((new Object() {
						...
						_datEntSetI.getEntityData().set(BabyNoarhornEntity.DATA_Age, (int) DoubleArgumentType.getDouble(arguments, "age"));
				} else {
					if ((new Object() {
						public Entity getEntity() {
							try {
					...
					}.getEntity()) instanceof BabySalmoranEntity _datEntSetI)
						_datEntSetI.getEntityData().set(BabySalmoranEntity.DATA_Age, (int) DoubleArgumentType.getDouble(arguments, "age"));
				}
				if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_COMMANDBLOCKOUTPUT)) {
					if (entity instanceof Player _player && !_player.level().isClientSide())
						_player.displayClientMessage(
								Component.literal((Component.translatable("event.distant_worlds.command.data_changed").getString() + "" + (new java.text.DecimalFormat("##").format(DoubleArgumentType.getDouble(arguments, "age")) + " / 7200"))),
								false);
				}
			} else {
				if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_COMMANDBLOCKOUTPUT)) {
