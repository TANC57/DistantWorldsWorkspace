...
public class GarhennaAdaptationGetCommandProcedure {
		...
		if (entity.hasPermissions(2)) {
			if ((new Object() {
				...
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal((Component.translatable("gui.distant_worlds.dalite_indicator_overlay.adaptation").getString() + "" + (new java.text.DecimalFormat("##").format(((new Object() {
						public Entity getEntity() {
							try {
								return EntityArgument.getEntity(arguments, "target");
							} catch (CommandSyntaxException e) {
					...
					}.getEntity()).getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).GarhennaAdaptation) + "" + ("/" + new java.text.DecimalFormat("##").format(5))))),
