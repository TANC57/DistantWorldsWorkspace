...
public class GarhennaSkyRenderProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		double ObjectsVisibility = 0;
		if ((entity.level().dimension()) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("distant_worlds:garhenna")))) {
			ObjectsVisibility = 1
					- ((entity.getCapability(DistantWorldsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DistantWorldsModVariables.PlayerVariables())).GarhennaDepletion / (double) ConfigCommonConfiguration.MAX_GARHENNA_DEPLETION.get())
							* 0.75;
			if (entity.getY() <= 90 && entity.getY() > 75) {
				ObjectsVisibility = ObjectsVisibility * Math.min((entity.getY() - 75) * 0.1, 1);
			} else if (entity.getY() <= 75) {
				ObjectsVisibility = 0;
			}
			...
			RenderSystem.enableCull();
			RenderSystem.enableDepthTest();
			renderDeepsky((int) (60 << 24 | (int) ((new Object() {
				public int get(LevelAccessor _world) {
					if (_world instanceof ClientLevel _level) {
						Vec3 _color = _level.getSkyColor(Minecraft.getInstance().gameRenderer.getMainCamera().getPosition(), Minecraft.getInstance().getPartialTick());
						return 255 << 24 | (int) (_color.x() * 255.0D) << 16 | (int) (_color.y() * 255.0D) << 8 | (int) (_color.z() * 255.0D);
					}
					return 0;
				}
			}).get(world)) << 16 | (int) ((new Object() {
				public int get(LevelAccessor _world) {
					if (_world instanceof ClientLevel _level) {
						Vec3 _color = _level.getSkyColor(Minecraft.getInstance().gameRenderer.getMainCamera().getPosition(), Minecraft.getInstance().getPartialTick());
						return 255 << 24 | (int) (_color.x() * 255.0D) << 16 | (int) (_color.y() * 255.0D) << 8 | (int) (_color.z() * 255.0D);
			...
			renderStars(500, 12345, 90, 0, 0, (int) ((int) (100 * ObjectsVisibility) << 24 | 255 << 16 | 213 << 8 | 74), false);
			{
				ResourceLocation _texturelocation = new ResourceLocation(("distant_worlds" + ":textures/" + "sun" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			renderTexture(30, 0, -15, 25, (int) ((int) (150 * ObjectsVisibility) << 24 | 255 << 16 | 255 << 8 | 255), false);
			{
				ResourceLocation _texturelocation = new ResourceLocation(("distant_worlds" + ":textures/" + "sun_small" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			renderTexture(30, 18, -13, 25, (int) ((int) (150 * ObjectsVisibility) << 24 | 255 << 16 | 255 << 8 | 255), false);
			renderTexture(40, 90, -22, 25, (int) ((int) (150 * ObjectsVisibility) << 24 | 255 << 16 | 255 << 8 | 255), false);
			RenderSystem.enableBlend();
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			if (entity.getY() < 0) {
				renderAbyss((int) (80 << 24 | 0 << 16 | 0 << 8 | 0), false);
			}
		}
	}
}
